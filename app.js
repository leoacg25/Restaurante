...
async function fetchExchangeRate(silent = false) {
  ...
  try {
    ...
    const rateData = data.monedas && data.monedas[currencyKey];
    const rate = rateData ? parseFloat(rateData.price) : null;
...
window.addEventListener('DOMContentLoaded', () => {
  initApp();
  checkLogin(); // Show login overlay if not authenticated
});
